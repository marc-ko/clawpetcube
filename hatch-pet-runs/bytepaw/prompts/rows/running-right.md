Create one horizontal animation strip for Codex pet `bytepaw`, state `running-right`.
...
Identity: same pet in every frame: Compact cube-shaped companion with tiny claw-machine arm, soft rounded firmware-board details, small LCD face, subtle maker-lab energy, no readable text or logos.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `3d-toy`: Stylized 3D toy mascot with smooth rounded forms, simple materials, clear silhouette, and no photoreal complexity. User style notes: Polished small 3D toy mascot, readable silhouette, teal/coral/charcoal accents, friendly embedded-hardware personality..
