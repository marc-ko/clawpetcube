Create Codex pet row `running-right` for `bytepaw`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact cube-shaped companion with tiny claw-machine arm, soft rounded firmware-board details, small LCD face, subtle maker-lab energy, no readable text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
