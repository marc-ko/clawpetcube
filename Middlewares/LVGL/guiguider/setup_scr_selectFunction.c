...
void setup_scr_selectFunction(lv_ui_selectFunction *ui)
{
	...
	lv_group_t *group = lv_group_create(); // Putting components in a group
	...
	lv_indev_set_group(indev_encoder, group); // Set the group to encoder

	lv_obj_set_scroll_snap_x(ui->selectFunction, LV_SCROLL_SNAP_CENTER); // Centered while scrolling
