...
//static uint8_t button_get_pressed_id(void);
...
//	button_init();
    ...
    /* 注册按钮输入设备，虚拟按下屏幕*/
//    lv_indev_drv_init(&indev_drv);						// 输入设备初始化
//    indev_drv.type = LV_INDEV_TYPE_BUTTON;				// 输入设备类型
//    indev_drv.read_cb = button_read;					// 设置输入设备回调函数
//    indev_button = lv_indev_drv_register(&indev_drv);	// 在LVGL中注册驱动程序，并保存创建的输入设备对象
...
//    if(btn_act >= 0) 	// 判断是否有按键按下
//	{
//        data->state = LV_INDEV_STATE_PR;	// 输入设备有按下
//        last_btn = btn_act;
//    } 
//	else 
//	{
